...
class SpiderTipsportDetailSpider(scrapy.Spider):
    ...
    async def parse(self, response):
        ...
        async with requests.AsyncSession(headers = headers, impersonate = 'safari') as session: # chrome
            yield_list_list = await self.fetch_all(session, list_of_items)
        for yield_list in yield_list_list:
            for yield_item in yield_list:
                yield yield_item

    async def fetch_all(self, s, list_of_items):
        tasks = []
        for item in list_of_items:
            task = asyncio.create_task(self.fetch(s, item))
            tasks.append(task)
        yield_list_list = await asyncio.gather(*tasks)
        return yield_list_list
    
    async def fetch(self, s, item):
        yield_list = []
        ...
        while isError and error_counter < 5:
            await asyncio.sleep(error_counter)
            response_get = await s.request("GET", url)
            try:
        ...
        try:
            ...
            yield_list.append({ # toto pak muzu asi smazaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaat
                'event_url': event_url,
                'error': True,
